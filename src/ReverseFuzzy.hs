...
-- brick needs these lens libraries

-- bib entries
-- parse a bib file (using parsec)
...
import Data.Char
import Data.List
import qualified Data.List as List (filter, intersperse, lookup)
...
import qualified Text.BibTeX.Entry as BE (T (..))
...
nextpdf p =
  let newcurrent =
      ...
      newprevious = reverse (_current p : reverse (_previous p))
   in PDFList
...
getnewpath e =
  let adt = getadt e
      ...
      nds = nodoublespace tolower
   in (spacetodash nds) ++ ".pdf"
...
mybibs :: [String]
mybibs =
  [ "/Users/ryan/onedrive/bibliographies/library.bib"
...
tophits s ss =
  let cutstring = words s
   in tophits' cutstring ss
...
data AppState e = AppState
  { _mysearchstate :: F.Form SearchString e Name
  , _mybibstate :: [String] --
  , _mypdflist :: PDFList
  , _mybestguess :: [BE.T]
...
initialState es xs =
  AppState
    { _mysearchstate = mkForm $ SearchState{_search = Text.pack " "}
    , _mybibstate = []
...
resetState st =
  AppState
    { _mysearchstate = resetformstate $ _mysearchstate st
...
reversefuzzy es s =
  let strs = map getadt2 es
      scores = map (rff [s]) strs
      mzip = zip scores es
      sorted = reverse $ sortOn fst mzip
   in map snd $ take 5 $ sorted
...
getadt2 :: BE.T -> [String]
getadt2 e =
  [ filter (not . isPunctuation) $ M.fromMaybe "" (getfield "author" e)
  , M.fromMaybe "" (getfield "date" e)
  , M.fromMaybe "" (getfield "title" e)
  ]
...
rfuzzy es s =
  let alltitlesubs = getalltitlesubs es
      fzscores = getfuzzyscores alltitlesubs s
      sorted = reverse $ sortOn fst fzscores
   in take 5 (map (fst . snd) sorted)
...
rfuzzy2 es s =
  let alltitlesubs = getalltitlesubs es
      fzscores = getfuzzyscores alltitlesubs s
      sorted = reverse $ sortOn fst fzscores
   in take 5 (map getadt (map (fst . snd) sorted))
...
getfuzzyscores es s =
  let fz = map (\x -> (Fuzzy.match (snd x) s "" "" id False)) es
   in zip (map extractscore fz) es
